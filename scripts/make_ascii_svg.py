...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "Kunj-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">kunj@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'kunj@github:~$ whoami <tspan fill="{INK}">Kunj Thummar</tspan></text>')
